...
import os
from analytics.trend_analyzer import TrendAnalyzer  # فقط برای داده نمونه
...
def run_suite_and_collect():
    suite = unittest.TestLoader().loadTestsFromTestCase(TestCSVExporter)
    stream = StringIO()
    runner = unittest.TextTestRunner(stream=stream, verbosity=2)
    result = runner.run(suite)
    output_text = stream.getvalue()
    expected_names = unittest.TestLoader().getTestCaseNames(TestCSVExporter)
...
def show_results_gui():
    ...
    root = tk.Tk(); root.title("CSV Exporter Test Results")
