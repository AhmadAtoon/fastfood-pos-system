...
class TestPrintTemplates(unittest.TestCase):
    def setUp(self):
        self.tsvc = TemplateService()
        self.psrv = PrintService()
        # قالب نمونه
        self.template = self.tsvc.create_template("simple_invoice", {
            "margins": {"top": 12, "left": 12, "right": 12, "bottom": 12},
            "header": {"lines": ["FastFood Co.", "123 Main St, City"], "align": "C"},
            "footer": {"lines": ["Thank you for your purchase!", "Tel: 021-123456"], "align": "C"},
            "table": {
                "columns": [
        ...
        self.invoice = {
            "header": {"order_id": "ORD-900", "customer_id": 500, "status": "paid", "created_at": 1732886400, "tax_rate": 9.0},
            "lines": [
                {"name": "Burger", "qty": 2, "price": 120000, "discount": 24000, "line_total": 216000},
                {"name": "Soda", "qty": 3, "price": 20000, "discount": 0, "line_total": 60000},
            ],
            "totals": {"subtotal": 276000, "discount_total": 24000, "tax": 22440, "grand_total": 274440}
        }

    def test_template_create_and_print(self):
        cfg = self.template["config"]
        out = self.psrv.print_invoice(self.invoice, "out_template_invoice.pdf", template_config=cfg)
        self.assertTrue(os.path.exists(out))

def run_suite_and_collect():
    suite = unittest.TestLoader().loadTestsFromTestCase(TestPrintTemplates)
    stream = StringIO()
    runner = unittest.TextTestRunner(stream=stream, verbosity=2)
    result = runner.run(suite)
    output_text = stream.getvalue()

    expected = unittest.TestLoader().getTestCaseNames(TestPrintTemplates)
...
def show_results_gui():
    ...
    root = tk.Tk()
    root.title("Print Templates Test Results")
