...
class TestTaxReports(unittest.TestCase):
    def setUp(self):
        ...
        self.reports = TaxReports(orders=[self.o1, self.o2], payments=[self.p1, self.p2])

    def test_tax_summary(self):
        summary = self.reports.generate_tax_summary()
        self.assertGreater(summary["gross_revenue"], 0)
        self.assertGreaterEqual(summary["net_revenue"], 0)

    def test_calculate_tax(self):
        tax = self.reports.calculate_tax(rate=0.09)
        self.assertGreater(tax, 0)

    def test_render_text_report(self):
        txt = self.reports.render_text_report(rate=0.09)
        self.assertIn("Tax Report", txt)
        self.assertIn("Tax Payable", txt)

# -------------------------------
# اجرای تست‌ها و نمایش گرافیکی
# -------------------------------
def run_suite_and_collect():
    suite = unittest.TestLoader().loadTestsFromTestCase(TestTaxReports)
    stream = StringIO()
    runner = unittest.TextTestRunner(stream=stream, verbosity=2)
    result = runner.run(suite)
    output_text = stream.getvalue()

    expected_names = unittest.TestLoader().getTestCaseNames(TestTaxReports)
...
def show_results_gui():
    ...
    root = tk.Tk()
    root.title("Tax Reports Test Results")
