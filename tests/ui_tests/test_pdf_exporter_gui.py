...
class TestPDFExporter(unittest.TestCase):
    def setUp(self):
        ...
        self.exporter = PDFExporter(self.filename)

    def test_export_creates_file(self):
        path = self.exporter.export(self.data, title="Sales Report")
        self.assertTrue(os.path.exists(path))

    def test_export_summary(self):
        path = self.exporter.export_summary("This is a summary report.", title="Summary")
        self.assertTrue(os.path.exists(path))

def run_suite_and_collect():
    suite = unittest.TestLoader().loadTestsFromTestCase(TestPDFExporter)
    stream = StringIO()
    runner = unittest.TextTestRunner(stream=stream, verbosity=2)
    result = runner.run(suite)
    output_text = stream.getvalue()
    expected_names = unittest.TestLoader().getTestCaseNames(TestPDFExporter)
...
def show_results_gui():
    ...
    root = tk.Tk(); root.title("PDF Exporter Test Results")
