...
class TestExcelExporter(unittest.TestCase):
    def setUp(self):
        ...
        self.exporter = ExcelExporter(self.filename)

    def test_export_creates_file(self):
        path = self.exporter.export(self.data)
        self.assertTrue(os.path.exists(path))

    def test_export_has_sheet(self):
        self.exporter.export(self.data, sheet_name="TestSheet")
        wb = load_workbook(self.filename)
        self.assertIn("TestSheet", wb.sheetnames)
    ...
    def test_export_with_styles(self):
        path = self.exporter.export_with_styles(self.data)
        self.assertTrue(os.path.exists(path))

def run_suite_and_collect():
    suite = unittest.TestLoader().loadTestsFromTestCase(TestExcelExporter)
    stream = StringIO()
    runner = unittest.TextTestRunner(stream=stream, verbosity=2)
    result = runner.run(suite)
    output_text = stream.getvalue()
    expected_names = unittest.TestLoader().getTestCaseNames(TestExcelExporter)
...
def show_results_gui():
    ...
    root = tk.Tk(); root.title("Excel Exporter Test Results")
